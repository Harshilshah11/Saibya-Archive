...
export default async function Home() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Robots"
        description="Sessions uploaded by each robot. Open one to watch the cameras or download the video and LiDAR + IMU data."
      >
          ...
          {robots.map((r) => {
            ...
            return (
              <Link
                ...
                className="group rounded-xl border border-border bg-panel p-4 shadow-card transition hover:-translate-y-px hover:border-accent/50 hover:shadow-card-hover"
              >
                <div className="flex items-center justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-3">
                    <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-panel-2 text-muted group-hover:text-accent">
                      <span className="brand-mark h-4" aria-hidden />
                    </span>
                    <div className="truncate font-mono text-lg font-semibold group-hover:text-accent">{r.robotId}</div>
                  </div>
                  {r.activeSessions > 0 ? (
